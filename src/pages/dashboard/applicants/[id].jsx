...
import { fetchApplicantById } from '@/backend/api';
import Loader from '@/components/common/Loader';
import ApplicantDetails from '@/components/dashboard/applicants/ApplicantDetails';
...
const DashboardApplicant = (props) => {
  const session = useSession();
  const router = useRouter();
  const id = props.id;

  const { applicant, applicantLoading, applicantError, mutateApplicant } = fetchApplicantById(id);
  ...
  if (session.status === 'authenticated') {
    return (
      <section style={dashBg}>
        <TopSection page="Applicant Details" />
        <div style={{ background: '#0F0113' }} className="py-5">
          <div className="text-center bx-container mt-5">
            <Row className="pb-3 px-3 px-md-0 justify-content-between">
              ...
              {/* Main content */}
              <Col sx={12} md={8} style={{ overflowY: 'scroll', maxHeight: '700px' }}>
                <Row className="pb-3 px-3 px-md-0">
                  ...
                  <div className="mb-3">
                    ...
                    {applicantLoading && <Loader />}
                    {applicantError && <div>{applicantError}</div>}
                  </div>
                </Row>
              </Col>
            </Row>
